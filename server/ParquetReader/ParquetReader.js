...
const readData = async () => {
    ...
    for (const dir of getDirectories(DATA_DIR)) {
        ...
        for (const gran of Object.keys(timeMap)) {
            grans[gran] = utils.computeGranularity(coins_dict[dir], timeMap[gran]).filter((dataPoint) => {return !Number.isNaN(dataPoint.y[0])})
        }
...
const filterData = (coins_dict) => {
    ...
    for (const dir of Object.keys(coins_dict)) {
        ...
        for (const gran of Object.keys(coins_dict[dir])) {
            filtered_coins_dict[dir][gran] = utils.computeMovingAverage(coins_dict[dir][gran])
        }
    }
